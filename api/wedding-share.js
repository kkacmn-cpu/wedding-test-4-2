...
function buildShareUrl(slug,version,baseUrl){
  const u=new URL('/api/wedding-share',baseUrl||FALLBACK_BASE_URL);
  u.searchParams.set('slug',slug);
  if(version)u.searchParams.set('v',String(version));
  return u.href;
}
...
function renderHtml({meta,cardUrl,status=200,message=''}){
  ...
  const canonical=esc(meta&&meta.url||cardUrl||CARD_BASE_URL);
...
async function handler(req,res){
  ...
  try{
    ...
    meta.url=buildShareUrl(slug,version,baseUrl);
    res.statusCode=200;
    if(req.method==='HEAD'){res.end();return;}
...
module.exports._test={esc,safeHttpUrl,formatDate,formatTime,buildOgImage,getCardImage,getNames,buildMeta,buildCardUrl,buildShareUrl,renderHtml,fetchCard,getRequestBaseUrl};
